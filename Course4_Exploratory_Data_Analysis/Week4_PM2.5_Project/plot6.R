...
make_plot6 <- function(){
  ...
  # Generate the subset of SCC related to motor vehicles and summarize emission
  # information across the US. Since which emission information is required is unclear,
  # I select the total emission from vehic* related data for Baltimore City
  summary5 <- get_total_emissions_by_type(NEI, list_of_scc_indices, "24510")
  summary5$Total_Emissions <- summary5$Total_Emissions/1000
  summary5$City <- "Baltimore city"
  
  # Repeat total emission calculation for Los Angeles City with fips = 06037
  summary6 <- get_total_emissions_by_type(NEI, list_of_scc_indices, "06037")
  summary6$Total_Emissions <- summary6$Total_Emissions/1000
  summary6$City <- "Los Angeles County"
  
  # Row bind summary5 and summary6 into finalsummary
  finalsummary <- rbind(summary5, summary6)
  
  # Plot summary
  g <- ggplot(finalsummary, aes(x=year, y=Total_Emissions, fill=City))
  g <- g + geom_bar(stat="identity", position="dodge")
  g <- g + xlab("Year")
  g <- g + ylab("Total PM2.5 Emissions from Motor Vehicles (kiloton)")
  g <- g + coord_cartesian(ylim=c(0,5))
  g <- g + ggtitle("Total Motor Vehicle PM2.5 Emission in Baltimore City and Los Angeles County")
  g <- g + scale_fill_brewer(palette="Dark2")
  #g <- g + geom_text(aes(label=round(Total_Emissions,2), vjust=-0.2, hjust=0.2))
